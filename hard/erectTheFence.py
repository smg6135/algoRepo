...
class Solution:
    def outerTrees(self, trees: List[List[int]]) -> List[List[int]]:
        # Explanation
        # We construct two fences -> one that goes over and one goes under
        # and finalize the fences by adding them together
        # using the idea where cross product can be used to determine whether the
        # next vector is on top or bottom, we construct the minimum fence
        # while calculating the cross product each time
